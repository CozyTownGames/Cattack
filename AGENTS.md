...
## Game Overview: "Cattack!"

This is a mobile-optimized game built for a Phaser contest on Devvit.
...
- Instead of `window.location` or `window.assign`, use `MapsTo` from `@devvit/web/client`
...
## Scope & Modification Boundaries (STRICT)

- **Zero Unsolicited Refactoring:** Do NOT clean up, optimize, modernize, or restructure surrounding code unless explicitly asked to do so.
- **Strict Scope Containment:** Modify ONLY the specific functions, methods, variables, or components mentioned in the prompt. Do not touch adjacent logic.
- **Preserve Existing Architecture:** Do not alter the underlying game loop, state management, or data schemas unless directly instructed.
- **No Feature Creep:** Do not add "bonus" features, extra UI elements, or unprompted animations/logic.
- **Ask Before Breaking:** If a requested change inherently breaks another part of the system or requires modifying an external file, you must explain the conflict and ASK for permission before writing the code. Do not assume you have permission to rewrite external dependencies.
...
- do not change anything that isnt explicitely asked of you. build on what exists without breaking what already works.
